...
```
~/Library/Mobile Documents/iCloud~md~obsidian/Documents/Obsidian Vault/Travel/首爾 釜山 Trip Planner/
```

Override with `TRIP_VAULT_DIR`. Because the vault sits in iCloud outside the repo, CI cannot read
it — that is why the `.md` files are committed to `src/content/trip/`.
...
The trip data used to come from a Notion export in which **every time ran one hour late** — naive
